...
class ClinicsServiceController extends Controller
{
    ...
    public function store(ClinicsServiceRequest $request)
    {
        ...
        if($data['system_service_id'] !== null){
    ...
    public function assign_doctor_list(Request $request)
    {
        ...
        if(auth()->user()->hasRole('doctor')){
            $query = DoctorServiceMapping::where('doctor_id',auth()->user()->id)->where('service_id',$request->service_id)->where('clinic_id', $request->clinic_id);
        }
        else{
            $query = DoctorServiceMapping::where('service_id',$request->service_id)->where('clinic_id', $request->clinic_id);
        }
