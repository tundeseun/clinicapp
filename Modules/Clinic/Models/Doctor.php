       ...
       if($user->hasRole('vendor')) {  
        ...
        if(auth()->user()->hasRole('receptionist')){
            ...
            if(multiVendor() == "0"){
                ...
                $query->where('vendor_id', $vendorId)->with('doctorService')->whereHas('doctorService', function ($query) use ($clinic_id) {
                    $query->where('clinic_id', $clinic_id);
                 });

             }else{

                  $query->with('doctorService')->whereHas('doctorService', function ($query) use ($clinic_id) {
                      $query->where('clinic_id', $clinic_id);
                   });
