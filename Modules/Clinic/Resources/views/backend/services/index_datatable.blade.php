...
<x-backend.advance-filter>
    ...
    <div class="form-group datatable-filter">
        ...
        <select name="price_range" id="price_range" class="form-control select2" data-filter="select">
            <option value="">{{ __('service.all') }} {{ __('service.lbl_price') }}</option>
            @foreach ($priceRanges as $range)
            <option value="{{ $range[0] }}-{{ $range[1] }}">{{ $range[0] }} - {{ $range[1] }}</option>
            @endforeach
        </select>
    </div>
    <div class="form-group datatable-filter">
...
<script type="text/javascript" defer>
    const columns = [
        @unless(auth()->user()->hasRole('receptionist') || auth()->user()->hasRole('doctor'))
        {
            name: 'check',
            data: 'check',
